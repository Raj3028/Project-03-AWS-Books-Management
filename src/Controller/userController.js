...
const createUser = async (req, res) => {
    ...
    try {
        ...
        if (!(title && name && phone && email && password)) { return res.status(400).send({ status: false, msg: "All Fields are Mandatory." }) }

        //=====================Validation of Title=====================//
        if (!checkString(title)) return res.status(400).send({ status: false, message: "Please Provide Title." })
        if (!validateTitle(title)) return res.status(400).send({ status: false, msg: "Invalid Title! Please put title between ('Mr' or 'Mrs' or 'Miss')." });
        ...
        if (!validateEmail(email)) return res.status(400).send({ status: false, msg: "Invalid EmailID Format." });

        //=====================Validation of Password=====================//
        if (!checkString(password)) return res.status(400).send({ status: false, message: "Please Provide Password." })
        if (!validatePassword(password)) return res.status(400).send({ status: false, msg: "Invalid Password Format." });
        ...
        if (address) {
            ...
            if (!validateName(address.street)) return res.status(400).send({ status: false, msg: "Invalid Street Address Format." });
        ...
        if (checkEmailPresent) return res.status(409).send({ status: false, msg: `This ${email} is already registered! Please Use Different EmailId for Registration.` });
        ...
        res.status(201).send({ status: true, msg: `${name} your registration sucessfully done.`, data: createUser })
...
const loginUser = async (req, res) => {
    ...
    try {
        ...
        //=====================Checking Mandotory Field=====================//
        if (!(email && password)) { return res.status(400).send({ status: false, msg: "All Fields are Mandatory(i.e. email & password)." }) }
